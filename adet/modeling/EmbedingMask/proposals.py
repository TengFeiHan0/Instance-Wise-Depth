...
class EmbedMaskPostProcessor(nn.Module):
    ...
    def forward_for_single_feature_map(
        ...
        for i in range(N):
            ...
            boxlist.add_field("pre_boxes",Boxes(detections)) 
    ...
    def forward_for_mask(self, boxlists, pixel_embed):
        ...
        for im in range(N):
            ...
            if len(proposal_embed) == 0:
                new_boxlist = Boxes(boxes, boxlist.size, mode="xyxy")
            ...
            box_masks = BitMasks(mask_boxes, m_h, m_w)
...
def make_embed_mask_postprocessor(cfg):
    pre_nms_thresh = cfg.MODEL.EMBED_MASK.INFERENCE_TH
    pre_nms_top_n = cfg.MODEL.EMBED_MASK.PRE_NMS_TOP_N
    nms_thresh = cfg.MODEL.EMBED_MASK.NMS_TH

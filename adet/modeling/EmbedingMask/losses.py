...
from adet.layers import (IOULoss, LovaszHinge)

from fvcore.nn import sigmoid_focal_loss_jit

from detectron2.structures import pairwise_iou,matched_boxlist_iou
...
class EmbedMaskLossComputation(object):
    ...
    def get_pos_proposal_indexes(self, locations, box_regression, matched_idxes, targets):
        ...
        for im in range(len(targets)):
            ...
            for t_id in range(len(targets[im])):
                ...
                detect_boxlist = Boxes(detections, targets[im].size, mode="xyxy")
                target_boxlist = Boxes(targets[im].bbox[t_id:t_id+1], targets[im].size, mode="xyxy")
                match_quality_matrix = pairwise_iou(detect_boxlist, target_boxlist)
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, proposal_embed, proposal_margin, pixel_embed, targets):
        ...
        box_cls_flatten,
        labels_flatten.int(),
        alpha=cfg.MODEL.EMBED_MASK.LOSS_ALPHA,
        gamma=cfg.MODEL.EMBED_MASK.LOSS_GAMMA,
        reduction="sum",
        ) / num_pos_avg_per_gpu
